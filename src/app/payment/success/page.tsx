    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 max-w-md w-full text-center">
        ...
        <div className="bg-gray-50 rounded-2xl p-4 mb-6 text-right space-y-2">
          {orderId && (
            <div className="flex justify-between text-sm">
              ...
              <span className="font-bold text-dark">{toPersianNumber(orderId)}</span>
            </div>
          )}
          {refId && (
            <div className="flex justify-between text-sm">
              <span className="text-gray-500">کد پیگیری:</span>
              <span className="font-bold text-dark font-mono">{refId}</span>
            </div>
          )}
          {gateway && (
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium text-dark">
                {gateway === "zarinpal" ? "زرین‌پال" : "آیدی‌پی"}
              </span>
            </div>
          )}
        </div>
        ...
        <div className="flex gap-3">
          <Link href="/account/orders" className="flex-1 btn-primary justify-center text-sm">
            <Package size={16} /> پیگیری سفارش
          </Link>
